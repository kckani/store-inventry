...
class Database:
    def __init__(self, db):
        ...
        self.cur.execute("CREATE TABLE IF NOT EXISTS Products (id INTEGER PRIMARY KEY, Product text, Prodtype text, AmountInStock text, Price text)")
        self.conn.commit()
    ...
    def insert(self, Product, Prodtype, AmountInStock, Price):
        self.cur.execute("INSERT INTO Products VALUES (NULL, ?, ?, ?, ?)", (Product, Prodtype, AmountInStock, Price))
        self.conn.commit()
    ...
    def update(self, id, Product, Prodtype, AmountInStock, Price):
        self.cur.execute("UPDATE Products SET Product = ?, Prodtype = ?, AmountInStock = ?, Price = ? WHERE id = ?", (Product, Prodtype, AmountInStock, Price, id))
        self.conn.commit()
...
def add_item():
    if Product_text.get() == '' or Prodtype_text.get() == '' or AmountInStock_text.get() == '' or Price_text.get() == '':
        messagebox.showerror('Required Fields', 'Please include all fields')
        return
    db.insert(Product_text.get(), Prodtype_text.get(), AmountInStock_text.get(), Price_text.get())
    Products_list.delete(0, END)
    Products_list.insert(END, (Product_text.get(), Prodtype_text.get(), AmountInStock_text.get(), Price_text.get()))
    clear_text()
    populate_list()
...
def select_item(event):
    try:
        ...
        AmountInStock_entry.insert(END, selected_item[3])
...
def update_item():
    db.update(selected_item[0], Product_text.get(), Prodtype_text.get(), AmountInStock_text.get(), Price_text.get())
    populate_list()
...
AmountInStock_label.grid(row=3, column=0, sticky=W)
AmountInStock_entry = Entry(app, textvariable=AmountInStock_text)
AmountInStock_entry.grid(row=3, column=1)
# Price
...
Products_list.grid(row=6, column=0, columnspan=3, rowspan=6, pady=20, padx=20)
# Create scrollbar
